  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      Promise.all(subjectIds.map(sid =>
        attendanceAPI.getPercentage(selected.id, sid).then(r => r.data.data).catch(()=>null)
              ...
              <div className="card">
                ...
                {attStats.length === 0 ? (
                  ...
                  <div key={s.subjectId} style={{ display:'flex', alignItems:'center',
                    ...
                    <span style={{ fontSize:13 }}>Subject #{s.subjectId}</span>
              ...
              <div className="card">
                ...
                {marks.length === 0 ? (
                  ...
                  <div key={m.id} style={{ display:'flex', justifyContent:'space-between',
                    ...
                    <div>
                      <div style={{ fontSize:13 }}>Subject #{m.subjectId}</div>
                      <div style={{ fontSize:11, color:'var(--text-muted)' }}>
                        {m.examType?.replace('_',' ')} · Sem {m.semester}
                      </div>
                    </div>
                    <div style={{ display:'flex', alignItems:'center', gap:8 }}>
