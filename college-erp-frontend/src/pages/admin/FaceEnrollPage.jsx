...
import { faceAPI, studentAPI, courseAPI } from '../../services/api';
...
export default function FaceEnrollPage() {
  ...
  React.useEffect(() => {
    if (!batchId) return;
    studentAPI.getByBatch(batchId).then(r => setStudents(r.data.data || []));
  }, [batchId]);
  ...
  const enroll = async () => {
    ...
    try {
      ...
      toast.success(`Enrolled ${res.data.data.enrolledImages} face images for ${selStudent.fullName}`);
  ...
  const trainModel = async () => {
    ...
    setTraining(true);
    try {
      const res = await faceAPI.train({ batchId: +batchId, studentIds: students.map(s => s.id) });
      toast.success(`Model trained with ${res.data.data.trainedSamples} samples`);
    } catch (err) {
    ...
    <div>
      <div className="page-header">
        <div className="page-header-left">
          ...
          <p>Enroll student faces and train the LBPH recognition model</p>
        </div>
      </div>
      ...
      <div className="card mb-6" style={{ background:'var(--accent-soft)', border:'1px solid rgba(79,110,247,0.15)' }}>
        ...
        <div style={{ display:'flex', gap:24, flexWrap:'wrap' }}>
          {[
            ['1','Select student','Choose a student from the batch list'],
            ['2','Upload photos','Upload 5–10 clear face photos with varied angles'],
            ['3','Enroll','Faces are detected and stored in the database'],
            ['4','Train model','Train the LBPH model for the batch after enrolling all students'],
            ['5','Take attendance','Faculty starts a live session and the camera recognises students automatically'],
          ].map(([n,t,d]) => (
      ...
      <div className="grid-2">
        ...
        <div className="card">
          ...
          <div className="form-group mb-4">
            ...
            <select className="form-select" value={batchId} onChange={e => { setBatchId(e.target.value); setSelStudent(null); }}>
        ...
        <div className="card">
          ...
          <div style={{ padding:20, background:'var(--bg)', borderRadius:8, marginBottom:20 }}>
            <div style={{ fontSize:13, fontWeight:500, marginBottom:8 }}>Batch: {batchId ? batches.find(b=>b.id==batchId)?.name : 'Not selected'}</div>
            <div style={{ fontSize:13, color:'var(--text-muted)', marginBottom:4 }}>Students: {students.length}</div>
          ...
          <div style={{ padding:14, background:'var(--amber-soft)', borderRadius:8, marginBottom:20 }}>
            <p style={{ fontSize:12.5, color:'#92400e' }}>
              ⚠ Train the model after enrolling all students in the batch. Re-train when new students are added.
              Training may take 30–60 seconds.
            </p>
          </div>
          <button className="btn btn-primary" onClick={trainModel} disabled={training || !batchId}
            style={{ width:'100%', justifyContent:'center' }}>
            {training ? <><span className="spinner"/>Training LBPH model…</> : '🧠 Train model for this batch'}
          </button>
